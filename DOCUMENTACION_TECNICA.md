...
La preferencia persistente del cartel de Inicio se almacena en
`identity.users.show_home_sell_banner boolean NOT NULL DEFAULT true`. Es propia de
cada cuenta y no se guarda en el JWT, las cookies ni el almacenamiento local del
navegador.
...
`PATCH /api/auth/preferences` recibe
`{ "show_home_sell_banner": false }` y responde `{ "user": <DTO> }`. Sólo
actualiza al usuario de la sesión actual; no regenera el JWT ni revoca sesiones.
El DTO nunca incluye `password_hash`.
